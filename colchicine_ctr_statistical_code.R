...
# =============================================================================
# R chunk: tumor-waterfall-plot  (, fig.width = 13, fig.height = 6.4, fig.cap=fig_cap("Best percent change in target-lesion sum of diameters during continued colchicine treatment. The waterfall plot shows the best percent change from baseline among five patients treated on the original schedule; the accompanying table summarizes scan timing, CRP change near the best-response scan, and prior immunotherapy exposure. One patient, ID 4, experienced tumor shrinkage of 19% after prior ipilimumab plus nivolumab, although CRP increased near the time of the scan. This post hoc analysis is hypothesis-generating only. Abbreviations: CRP, C-reactive protein; IO, immunotherapy; SOD, sum of diameters."))
# =============================================================================
suppressPackageStartupMessages({
...
if (nrow(tumor_waterfall_dat) > 0) {
  p_tumor_waterfall <- ggplot(
    ...
    labs(
      title = "Best tumor change during continued colchicine treatment (n = 5)",
      x = "Study ID",
      y = "Best % SOD change"
    ) +
  ...
  tumor_annot_tbl <- tumor_waterfall_dat %>%
    ...
    transmute(
      ...
      `Best SOD change` = sprintf("%+.0f%%", best_pct_change),
      `1st scan` = dplyr::coalesce(first_scan_cd, "NE"),
      `Best scan` = dplyr::coalesce(scan_cd, "NE"),
      `CRP near best scan` = crp_change_lab,
  ...
  fig_caption_grob <- grid::textGrob(
    paste0(
      "Exploratory analysis of patients treated on the original schedule. ",
      "Blue indicates tumor shrinkage; orange indicates tumor growth. ",
      "CRP reflects the measurement nearest the best-response scan within +/-21 days.\n",
      "*ID 4: prior ipilimumab + nivolumab. C#D# = calendar days from that cycle IP start."
    ),
