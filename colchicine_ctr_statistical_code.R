...
# =============================================================================
# R chunk: tumor-waterfall-plot  (, fig.width = 12, fig.height = 11.5, fig.cap=fig_cap("Best percent change in target-lesion sum of diameters (SOD) among patients with ≥2 on-study scans (n = 5). All were on the original continued-colchicine schedule (not the amended 2-week course). Labels show CRP at baseline versus at the best-response scan, first versus best scan timing (cycle/day), and prior immunotherapy when applicable. Abbreviations: SOD, sum of diameters; CRP, C-reactive protein; IO, immunotherapy."))
# =============================================================================
...
tumor_waterfall_dat <- tumor_change %>%
  ...
  mutate(
    ...
    timing_lab = if_else(
      best_is_first_scan,
      paste0(
        "1st/best scan ", dplyr::coalesce(scan_cd, "NE"),
        "\nCRP draw ", dplyr::coalesce(crp_cd, "NE")
      ),
      paste0(
...
tumor_fig_caption <- paste0(
  ...
  "Each bar = best tumor-size change. Labels: CRP baseline → at best-scan CRP draw; 1st vs best scan timing (cycle/day); prior IO if any."
)
