...
All `r consented_n` subjects who signed informed consent underwent screening evaluation. **During trial conduct, a protocol amendment limited on-study colchicine to a fixed 2-week course for patients enrolled under the amended schedule; patients enrolled under the original schedule received indefinite on-study colchicine with serial imaging surveillance.** `r enrolled_original_n` enrolled patients are included in Kaplan–Meier progression-free and overall survival analyses; `r enrolled_amended_n` enrolled after the amendment contributed to toxicity and primary CRP analyses but were excluded from survival estimates because they did not follow the same longitudinal imaging schedule. When organ-function or CRP thresholds were not met, the report states the clinical reason for screen failure and the screening laboratory value when available.
...
**Analyses in this section include only the `r km_eligible_n` patients enrolled under the original indefinite colchicine schedule with serial imaging surveillance.** The `r enrolled_amended_n` patients enrolled after the protocol amendment (fixed 2-week intervention) lacked the same longitudinal imaging framework and are reported for CRP pharmacodynamics and safety only.
...
`r if (params$public_site) "*Statistical methods follow protocol v3.0 Section 12.4.*" else "*Internal analytic report. Verify all numbers against source data and principal investigator review before journal submission.*"`
