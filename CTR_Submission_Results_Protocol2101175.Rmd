...
params:
  public_site: false
title: "Colchicine Pilot Trial — Statistical Results"
subtitle: "`r if (!params$public_site) 'Clinical Trial Results (The Oncologist CTR format)' else ''`"
...
`r if (!params$public_site) knitr::knit_child("sections/inline-edit.Rmd")`

`r if (params$public_site) knitr::knit_child("sections/public-header.Rmd") else knitr::knit_child("sections/ctr-header.Rmd")`

`r if (!params$public_site) knitr::knit_child("sections/abstract.Rmd")`

`r if (!params$public_site) knitr::knit_child("sections/lessons-learned.Rmd")`

`r if (!params$public_site) knitr::knit_child("sections/trial-information.Rmd")`

`r if (!params$public_site) knitr::knit_child("sections/drug-information.Rmd")`
...
All `r consented_n` subjects who signed informed consent underwent screening evaluation. **Protocol v3.0 specified two Cohort 1 treatment schedules: the original schedule (indefinite on-study colchicine with serial imaging surveillance) and a fixed 2-week course without transitioning patients already on the original schedule.** `r enrolled_original_n` enrolled patients are included in Kaplan–Meier progression-free and overall survival analyses; `r enrolled_amended_n` enrolled on the fixed 2-week schedule contributed to toxicity and primary CRP analyses but were excluded from survival estimates because they did not follow the same longitudinal imaging schedule. When organ-function or CRP thresholds were not met, the report states the clinical reason for screen failure and the screening laboratory value when available.
...
Progression-free survival (PFS) and overall survival (OS) were **not recorded on the dedicated case report forms** in the available dataset. Endpoints below were derived per patient from: (1) RECIST 1.1 best overall response (progressive disease [PD] date); (2) protocol therapy discontinuation for documented progression; (3) final subject status and date of death or last contact. Time zero = first investigational product dispense date, or enrollment if missing.

**Analyses in this section include only the `r km_eligible_n` patients enrolled under the original indefinite colchicine schedule with serial imaging surveillance.** The `r enrolled_amended_n` patients enrolled on the fixed 2-week schedule lacked the same longitudinal imaging framework and are reported for CRP pharmacodynamics and safety only.
...
### Exploratory and safety analyses

Exploratory progression-free and overall survival estimates used the Kaplan–Meier method among the `r km_eligible_n` patients enrolled under the original indefinite-treatment schedule with serial imaging surveillance only. PFS and OS were derived from RECIST 1.1 assessments, protocol therapy discontinuation for progression, and final subject status when dedicated progression forms were incomplete. Treatment-related adverse events required Possible, Probable, or Definite attribution to colchicine and were summarized by CTCAE v5.0 preferred term and worst grade per patient. Analyses followed [protocol `r protocol_id`, version `r protocol_version`](`r nct_url`) Section 12. [Supplementary statistical methods and analysis output](https://alexanderb95-jpg.github.io/colchicine-ctr-stats/statistical-analysis.html) are available separately.

---

`r if (!params$public_site) knitr::knit_child("sections/discussion.Rmd")`

`r if (!params$public_site) knitr::knit_child("sections/back-matter.Rmd")`

`r if (!params$public_site) knitr::knit_child("sections/figures-and-tables.Rmd")`
...
`r if (params$public_site) paste0("*Statistical methods follow [protocol ", protocol_id, ", version ", protocol_version, "](", nct_url, ") Section 12.4.*") else "*Internal analytic report. Verify all numbers against source data and principal investigator review before journal submission.*"`
