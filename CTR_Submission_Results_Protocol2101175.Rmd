...
The primary endpoint was tested per protocol Section 12.4 at prespecified **H0: mean maximum CRP decline = 0** versus **H1: mean decline > 0** (one-sided), with **α = 0.025** after Bonferroni adjustment for two planned Cohort 1 tests; only the low-dose one-sample test was performed because the high-dose arm did not enroll.
...
**Primary analysis (Cohort 1 low dose).** The primary endpoint was maximum percent decline in peripheral blood CRP from cycle 1 day 1 to the lowest on-treatment value during post-baseline cycle 1 visits (C1D8/C1D15). We tested the prespecified null hypothesis **H0: mean maximum CRP decline = 0** against the one-sided alternative **H1: mean decline > 0** using a one-sample *t*-test. Per protocol Section 12.4, the overall Cohort 1 type I error was controlled at 0.05 across **two** planned primary tests (low-dose vs 0 and high-dose vs low-dose) using **Bonferroni’s method**, yielding **α = 0.025 for each test**. Because the high-dose sequential arm did not enroll, only the low-dose one-sample test was performed at α = 0.025. The observed mean decline was `r fmt_num(primary_mean)`% (SD `r fmt_num(primary_sd)`%); one-sided *p* = `r fmt_p(primary_p)`, **which does not reject H0 at the prespecified α = 0.025** (i.e., the primary endpoint was not met because *p* > 0.025, not because *p* > 0.05).

With `r evaluable_efficacy_n` evaluable patients instead of the planned 15 per low-dose group, the completed analysis was underpowered to confirm the observed effect at α = 0.025 (post-hoc power ≈ `r fmt_num(posthoc_power_pct, 0)`% for Cohen’s d ≈ `r fmt_num(cohens_d_primary, 2)`).

*Contextual note:* With only one of two Bonferroni-planned tests executed, α = 0.025 is conservative relative to a single-test α = 0.05; this does not revise the prespecified conclusion.
