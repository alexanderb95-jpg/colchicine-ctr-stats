...
**Background:** Pro-tumorigenic inflammation in solid tumors is linked to IL-1β–NLRP3 signaling and elevated C-reactive protein (CRP). Preclinical and translational work, including studies of myeloid inflammation signatures in urothelial carcinoma (e.g., Tran et al., [PMC13069984](https://pmc.ncbi.nlm.nih.gov/articles/PMC13069984/)), supported targeting this axis with colchicine. We conducted a single-center pilot trial to estimate the pharmacodynamic effect of colchicine on CRP in advanced solid tumors and high-risk urothelial carcinoma.

**Methods:** Open-label, non-randomized pilot (planned N = 45). Cohort 1: metastatic/recurrent solid tumors with CRP > 5 mg/L — colchicine 0.6 mg twice daily (BID) or three times daily (TID) for 14 days. Cohort 2: resected high-risk urothelial carcinoma — colchicine 0.6 mg BID for 28 days with adjuvant nivolumab. Primary endpoint: maximum percent decline in CRP from cycle 1 day 1 during on-treatment assessment. Safety per CTCAE v5.0.

**Results:** `r consented_n` patients signed informed consent; `r enrolled_n` enrolled on study and `r screen_fail_n` failed screening before enrollment (Accrual Disposition). All enrolled patients were Cohort 1 low-dose BID. Cohort 1 high-dose and Cohort 2 did not accrue (Cohort 2 closed for poor accrual). `r evaluable_efficacy_n` patients were evaluable for the primary endpoint. Mean maximum CRP decline was `r fmt_num(primary_mean)`% (SD `r fmt_num(primary_sd)`%); one-sided one-sample *t*-test *p* = `r fmt_p(primary_p)`. Treatment-related adverse events were predominantly grade 1–2. Exploratory progression-free survival (PFS) and overall survival (OS) were estimable from RECIST assessments and status records: `r n_pfs_events`/`r nrow(surv_pfs)` PFS events and `r n_dead`/`r nrow(surv_os)` deaths (not powered; dedicated PFS/OS forms were largely incomplete).

**Conclusion:** In this under-accrued pilot, colchicine 0.6 mg BID was feasible in Cohort 1 with a modest pharmacodynamic signal on CRP in evaluable patients. The trial did not complete planned multi-cohort enrollment; findings are hypothesis-generating. ClinicalTrials.gov identifier: [to be completed].

**Key words:** colchicine; C-reactive protein; inflammation; solid tumors; urothelial carcinoma; pilot trial

---

## Lessons Learned

- **Accrual:** Cohort 2 (adjuvant urothelial carcinoma) closed early because of poor accrual; results should not be interpreted as lack of efficacy of the strategy in that setting.
- **Operational:** Only the Cohort 1 low-dose (BID) sequence enrolled (`r enrolled_n`/`r 30` planned for Cohort 1); high-dose TID and Cohort 2 were not evaluable in this dataset.
- **Endpoint:** Serial CRP collection at protocol-defined visits is essential; `r enrolled_n - evaluable_efficacy_n` enrolled patient(s) lacked sufficient on-treatment CRP pairs for the primary analysis.
...
All `r consented_n` subjects who signed informed consent underwent screening evaluation. Enrollment status was recorded on the REDCap screening form (`was_the_subject_enrolled`: 1 = enrolled, 0 = screen failure). When organ-function or CRP thresholds were not met, the report states the clinical failure (e.g., inadequate organ function) and the screening laboratory value when available. REDCap may classify these as “inclusion criteria” screen failures because adequate organ function and CRP elevation are protocol inclusion requirements.
...
```{r km-os, fig.cap=fig_cap("Overall survival (Kaplan–Meier) from treatment start (exploratory; not powered). Abbreviations: OS, overall survival."), eval=!is.null(fit_os)}
if (!is.null(fit_os)) {
  ggsurvplot(
    fit_os,
    data = surv_os,
...
*These survival analyses are descriptive and underpowered (pilot N = `r enrolled_n`). Medians and curves should be interpreted cautiously; several patients lack formal final-status forms but have tumor assessment or visit dates used for censoring.*

---

## Discussion

This pilot trial (STUDY-21-001175) was designed to test whether short-course colchicine suppresses peripheral blood CRP as a pharmacodynamic marker of pro-tumorigenic inflammation in advanced solid tumors and in high-risk urothelial carcinoma receiving adjuvant nivolumab. The scientific premise builds on NLRP3–IL-1β biology and translational studies linking myeloid inflammation to checkpoint inhibitor resistance, including work by Tran and colleagues ([PMC13069984](https://pmc.ncbi.nlm.nih.gov/articles/PMC13069984/)) that informed the protocol’s correlative agenda.

**Accrual and conduct.** Only the Cohort 1 low-dose (colchicine 0.6 mg BID) group enrolled (`r enrolled_n` patients). The sequential high-dose cohort and Cohort 2 did not enroll in this dataset; **Cohort 2 was closed because of poor accrual**, not because of a pre-specified efficacy or safety stopping rule. This limits generalizability to the adjuvant urothelial setting and precludes the planned dose-comparison and Cohort 2 primary analysis.

**Primary endpoint.** Among `r evaluable_efficacy_n` evaluable patients, the mean maximum CRP decline was `r fmt_num(primary_mean)`% (SD `r fmt_num(primary_sd)`%). The prespecified one-sided one-sample *t*-test at α = 0.025 yielded *p* = `r fmt_p(primary_p)`, which does not meet the protocol’s threshold for concluding a statistically significant anti-inflammatory effect in this small, incomplete sample. Individual responses varied, and some patients had rising on-treatment CRP, emphasizing heterogeneity in a heavily pretreated population.

**Safety.** Adverse events were manageable and predominantly low grade, consistent with known colchicine toxicities (gastrointestinal symptoms). Attribution to colchicine should be interpreted cautiously in patients with advanced cancer and concurrent comorbidities.

**Clinical implications.** These results do not establish clinical benefit on tumor endpoints. They provide feasibility and preliminary pharmacodynamic data for colchicine 0.6 mg BID in Cohort 1 only. Whether CRP suppression translates to improved outcomes with systemic therapy remains untested. The trial illustrates the CTR reporting value of publishing under-accrued pilots with explicit lessons learned.

**Next steps.** Future studies would require successful accrual to planned cohorts, standardized CRP sampling, and integration with outcome and correlative endpoints (ctDNA, myeloid signatures) prespecified in the protocol.
