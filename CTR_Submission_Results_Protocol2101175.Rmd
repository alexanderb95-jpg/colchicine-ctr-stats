...
```{r tumor-waterfall-plot, fig.width = 12, fig.height = 11.5, fig.cap=fig_cap("Best percent change in target-lesion sum of diameters (SOD) among patients with ≥2 on-study scans (n = 5). All were on the original continued-colchicine schedule (not the amended 2-week course). Labels show CRP at baseline versus at the best-response scan, first versus best scan timing (cycle/day), and prior immunotherapy when applicable. Abbreviations: SOD, sum of diameters; CRP, C-reactive protein; IO, immunotherapy.")}
